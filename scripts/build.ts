...
async function BuildBrowserExt(browsers: string[]) {
  ...
  for (const matchedBrowser of matchedBrowsers) {
    ...
    let data = JSON.parse(fs.readFileSync(resolve(inputDir, "manifest.json"), 'utf-8'));
    const app = data.name,
      v = data.version;
    // console.log(data)
    const noutDir = resolve(OutDir, toKebabCase(matchedBrowser.name + '_' + app + '_' + v))
    fse.copySync(inputDir, noutDir);
  }
